...
import { usePartnerProfile, type ProfileField } from './usePartnerProfile'
import styles from './PartnerProfile.module.css'

/*
 * PartnerProfile (page) — 파트너 · 내 권한/설정 · 프로필 정보
 * ------------------------------------------------------------------
 * 리더 프로필과 동일 구성(상태 요약 + 내 코드 히어로 + A/B/D 섹션)이며
 * '내 코드'만 파트너 값(NG-SP-0004)으로 다르다. UI 텍스트는 profile.* 재사용.
 * 플로우: [본사 승인 요청] → 확인 모달 → 완료 토스트. 입력은 UI 상태만.
 */
export default function PartnerProfile() {
  const { t } = useTranslation()
  const { statusItems, code, accountFields, basicFields } = usePartnerProfile()
